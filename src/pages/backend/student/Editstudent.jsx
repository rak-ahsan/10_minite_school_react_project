...
const Editstudent = () => {
  ...
  const update = (e) => {
    ...
    axios.put(`http://localhost/ReactProject/api/api-update-student.php?id=${id}`, data).then(res => {
      console.log(res);
            alert("successful");
        });
  }
  ...
  useEffect(() => {
    axios.get(`http://localhost/ReactProject/api/api-fetch-single-student.php?id=${id}`).then(res => {
        console.log(res);
        setStudent(res.data)
          ...
          <div className="container mt-3 justify-content-center d-flex">
            <form className="row g-3" onSubmit={update} >
              ...
              <div className="mb-3 col-md-6">
                ...
                <Form.Select aria-label="Default select example" value={student.studentclass} name='studentclass' onChange={input}>
              ...
              <Bntsubmit text="Update Student" className="btn-success" type="submit" />
            </form>
          </div>
        </div >
      </div >
